<x-app-layout>
    <div class="max-w-7xl mx-auto px-4 lg:px-8">
        ...
        <div class="flex justify-end">
            <div class="h-6 px-2 bg-white rounded-md shadow-md mb-2 divide-x flex items-center mr-4">
                <!-- ソート方法のドロップダウン -->
                    ...
                    <x-dropdown align="left" width="48">
                        <x-slot name="trigger">
                            <button class="flex items-center text-xs font-medium text-gray-500 hover:text-gray-700 hover:border-gray-300 focus:outline-none focus:text-gray-700 focus:border-gray-300 transition duration-150 ease-in-out">
                                <div>{{ App\Consts\SortOptConst::SORT_LIST[session('sort_option')] }}</div>
                        ...
                        <x-slot name="content">
                            @foreach (App\Consts\SortOptConst::SORT_LIST as $key => $value)
                            <form method="GET" action="{{ url('/') }}">
                                @csrf
                                <input type="hidden" name="sort_option" value="{{ $key }}">
